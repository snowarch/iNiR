...
```bash
./setup update        # Проверка удалённого репо, загрузка, синхронизация, перезапуск шелла
```

Или запустите `./setup` без аргументов для интерактивного TUI-меню, где можно обновить, мигрировать, откатить, диагностировать и многое другое.

Ваши конфиги не трогаются. Новые фичи приходят как опциональные миграции. Откат включён на случай проблем (`./setup rollback`).
