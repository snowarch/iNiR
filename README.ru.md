...
<p align="center">
  ...
  <sub>Изначально форк inir от end-4 — вырос в самостоятельный проект</sub>
</p>
...
<details open>
<summary><b>Material inir</b> — плавающая панель, сайдбары, эстетика Material Design</summary>
...
- **Material inir** — плавающая панель (сверху/снизу, 4 стиля углов), сайдбары, док (все 4 позиции), панель управления, вертикальная панель
- **Waffle** — панель задач в стиле Windows 11, стартовое меню, центр действий, центр уведомлений, панель виджетов, обзор задач

### Сайдбары и виджеты (Material inir)
...
```bash
qs log -c inir                    # Проверьте логи — ответ обычно там
qs kill -c inir && qs -c inir       # Перезапустить шелл
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — оригинальный inir для Hyprland, с чего всё началось
