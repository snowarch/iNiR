...
PanelWindow {
    ...
    GlassBackground {
        ...
        ColumnLayout {
            ...
            // Persist the chosen "stay unlocked" duration at the actual unlock
            // (not live while dragging the slider), then unlock the vault.
            function doUnlock(pw) {
                Config.setNestedValue("keepass.cacheTtl", ttlSlider.value)
                KeePass.unlock(pw)
            }
            ...
            Keys.onPressed: event => {
                ...
                if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                    if (!KeePass.unlocked) {
                        panelColumn.doUnlock(unlockPassword.text)
                        unlockPassword.text = ""
                        event.accepted = true
            ...
            Rectangle {
                ...
                ColumnLayout {
                    ...
                    ToolbarTextField {
                        ...
                        echoMode: TextInput.Password
                        onAccepted: { panelColumn.doUnlock(text); text = "" }
                    }
                    ...
                    RowLayout {
                        ...
                        StyledText {
                            text: KeePass.unlocked
                                ...
                                : Translation.tr("Stay unlocked: %1 min").arg(Math.floor(ttlSlider.value / 60))
                            font.pixelSize: Appearance.font.pixelSize.small
                    ...
                    Slider {
                        ...
                        to: 14400
                        stepSize: 60
                        // Seeded from the persisted value; dragging only moves
                        // the handle — the value is committed on actual unlock.
                    ...
                    RowLayout {
                        ...
                        DialogButton {
                            ...
                            activeFocusOnTab: false
                            onClicked: { panelColumn.doUnlock(unlockPassword.text); unlockPassword.text = "" }
                        }
                        DialogButton {
                            buttonText: Translation.tr("Cancel")
                            activeFocusOnTab: false
            ...
            ColumnLayout {
                ...
                ListView {
                    ...
                    // Auto-select the highlighted entry while navigating so the
                    // detail card is already open: one Enter copies (no double
                    // press) and Alt reveals the entry you're looking at.
                    function selectCurrent() {
                        if (!activeFocus || currentIndex < 0 || currentIndex >= count) return
                        const entry = model[currentIndex]
                        if (entry !== undefined && KeePass.selectedEntry !== entry)
                            KeePass.openEntry(entry)
                    }
                    onActiveFocusChanged: if (activeFocus) selectCurrent()
                    ...
                    Keys.onPressed: event => {
                        if (event.key === Qt.Key_Down) {
                            ...
                            else { currentIndex = Math.max(0, currentIndex - 1); selectCurrent() }
                            event.accepted = true
