...
def remove_conflicting_theme_imports(filepath, patterns_to_remove):
    """Remove lines matching patterns (e.g., omarchy theme imports) from config file."""
    filepath = Path(filepath)
...
def fix_foot_config(config_path):
    """Remove conflicting [colors] section from foot.ini (colors come from include)."""
    try:
    ...
    if "omarchy" in content.lower():
        content = re.sub(
            r"^.*omarchy.*$\n?", "", content, flags=re.MULTILINE | re.IGNORECASE
        )

    if re.search(r"^\[colors\]\s*\n\s*alpha\s*=", content, re.MULTILINE):
        content = re.sub(
            r"^\[colors\]\s*\n(\s*alpha\s*=[^\n]*\n?)+",
            "",
            content,
...
def fix_ghostty_config(config_path):
    """Remove omarchy config-file import from ghostty config."""
    patterns = [
        r"^.*config-file\s*=.*omarchy.*$\n?",
    ]
    return remove_conflicting_theme_imports(config_path, patterns)


def fix_alacritty_omarchy_import(config_path):
    """Replace omarchy import with iNiR colors.toml import in alacritty config."""
    try:
        with open(config_path, "r") as f:
            content = f.read()
    except FileNotFoundError:
    ...
    if "omarchy" in content.lower():
        content = re.sub(
            r'(general\.)?import\s*=\s*\[\s*["\'].*omarchy.*["\']\s*\]',
...
"""
    ...
    fix_alacritty_omarchy_import(alacritty_conf)
...
"""
    ...
    fix_ghostty_config(ghostty_conf)
