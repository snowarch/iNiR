...
def generate_kitty_config(colors, output_path):
    ...
    config = f"""# Auto-generated by inir wallpaper theming system
...
"""
    ...
    if ensure_line_in_file(ghostty_conf, "theme = inir-auto", r"theme\s*=\s*inir-auto"):
...
[General]
Description=inir Auto-generated Theme
Opacity=1.0
Wallpaper=
"""
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
"""
    ...
    if os.path.exists(starship_conf):
        ...
        # Check if palette_name is already set to inir
        if 'palette = "inir"' in content:
            print(f"✓ Generated Starship palette (already using inir palette)")
        else:
            # Add palette directive if not present
            if 'palette =' not in content:
                # Add at top of file
                new_content = 'palette = "inir"\n\n' + content
                Path(starship_conf).write_text(new_content)
                content = new_content
                print(f"✓ Generated Starship palette and set as active")
            else:
                print(f"✓ Generated Starship palette (using different palette, change to 'palette = \"inir\"' to use)")
        
        # Update or append the [palettes.inir] section in starship.toml
        # Starship doesn't support source/include, so palette must be inline
        if '[palettes.inir]' in content:
            # Replace existing palette section with updated colors
            # Find start of [palettes.inir] and end (next section header or EOF)
            pattern = r'\[palettes\.inir\].*?(?=\n\[|\Z)'
            # Extract just the palette block from the generated config
            palette_block = config.strip().split('\n')
            # Skip comment lines at the top, keep from [palettes.inir] onward
            palette_start = next(i for i, line in enumerate(palette_block) if line.startswith('[palettes.inir]'))
            palette_content = '\n'.join(palette_block[palette_start:])
            new_content = re.sub(pattern, palette_content, content, flags=re.DOTALL)
            if new_content != content:
                Path(starship_conf).write_text(new_content)
                print(f"  → Updated inir palette in starship.toml")
        else:
            with open(starship_conf, 'a') as f:
                f.write('\n' + config)
            print(f"  → Appended inir palette to starship.toml")
    else:
        print(f"✓ Generated Starship palette (starship.toml not found - create it and add 'palette = \"inir\"')")
...
"""
    ...
    if btop_path.exists():
        ...
        new_line = 'color_theme = "inir-auto"'
        if re.search(r'^color_theme\s*=', content, re.MULTILINE):
            ...
            if new_content != content:
                ...
                print(f"\u2713 Generated btop theme (already using inir-auto)")
        else:
        ...
        btop_path.write_text(f'color_theme = "inir-auto"\n')
...
"""
    ...
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, "w") as f:
        f.write(f"# Auto-generated by inir wallpaper theming system\n")
...
def generate_yazi_config(colors, output_path):
    """Generate yazi flavor for inir theming.
    
    Creates a flavor directory at ~/.config/yazi/flavors/inir-auto.yazi/
...
"""
    ...
    flavor_line = 'use = "inir-auto"'
    if theme_path.exists():
        content = theme_path.read_text()
        if '[flavor]' in content:
            if re.search(r'^use\s*=\s*"inir-auto"', content, re.MULTILINE):
                print(f"\u2713 Generated yazi flavor (already using inir-auto)")
            elif re.search(r'^use\s*=', content, re.MULTILINE):
                new_content = re.sub(r'^(use\s*=).*$', f'use = "inir-auto"', content, count=1, flags=re.MULTILINE)
...
def main():
    ...
    if "ghostty" in terminals:
        generate_ghostty_config(colors, f"{home}/.config/ghostty/themes/inir-auto")

    if "konsole" in terminals:
        generate_konsole_config(
            colors, f"{home}/.local/share/konsole/inir-auto.colorscheme"
        )

    if "starship" in terminals:
        generate_starship_config(colors, f"{home}/.config/starship/inir-palette.toml")

    if "btop" in terminals:
        generate_btop_config(colors, f"{home}/.config/btop/themes/inir-auto.theme")

    if "lazygit" in terminals:
        generate_lazygit_config(colors, f"{home}/.config/lazygit/inir-theme.yml")

    if "yazi" in terminals:
        generate_yazi_config(colors, f"{home}/.config/yazi/flavors/inir-auto.yazi/flavor.toml")
