...
check_zoxide_init() {
    ...
    if ! command -v zoxide &>/dev/null; then
        doctor_pass "zoxide (not installed, skipping)"
        return 0
    fi
...
ZEOF
    ...
    if ! [[ -f "$fish_conf" ]]; then
        ...
        cat > "$fish_conf" << FEOF
# iNiR zoxide init — auto-generated by doctor
...
FEOF
    ...
    if [[ $fixed -gt 0 ]]; then
        doctor_fix "Added zoxide init to $fixed shell profile(s)"
    else
        doctor_pass "zoxide init OK"
    fi
}
...
tui_step 19 $total_steps "Checking zoxide init"
check_zoxide_init
