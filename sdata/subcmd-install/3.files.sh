...
#####################################################################################
# Zoxide init (smart cd — provides 'z' command by default, 'cd' if opted in)
#####################################################################################
tui_info "Configuring zoxide..."

ZOXIDE_CMD_FLAG=""
if command -v jq &>/dev/null && [[ -f "$(inir_config_file 2>/dev/null)" ]]; then
    if [[ "$(jq -r '.apps.zoxideAsCd // false' "$(inir_config_file)" 2>/dev/null)" == "true" ]]; then
        ZOXIDE_CMD_FLAG="--cmd cd"
    fi
fi

# Bash: source zoxide init from .bashrc
ZOXIDE_MARKER="# iNiR zoxide init"
if [[ -f "$HOME/.bashrc" ]]; then
    sed -i "/${ZOXIDE_MARKER}/,/# end iNiR zoxide/d" "$HOME/.bashrc"
fi
cat >> "$HOME/.bashrc" << ZOXBEOF

${ZOXIDE_MARKER}
if command -v zoxide &>/dev/null; then
  eval "$(zoxide init bash${ZOXIDE_CMD_FLAG:+ }${ZOXIDE_CMD_FLAG})"
fi
# end iNiR zoxide
ZOXBEOF
log_success "Bash zoxide configured"

# Fish: conf.d snippet
FISH_CONF_DIR="${XDG_CONFIG_HOME}/fish/conf.d"
mkdir -p "$FISH_CONF_DIR"
cat > "${FISH_CONF_DIR}/inir-zoxide.fish" << ZOXFEOF
# iNiR zoxide init — auto-generated by setup
if command -v zoxide &>/dev/null
    zoxide init fish${ZOXIDE_CMD_FLAG:+ }${ZOXIDE_CMD_FLAG} | source
end
ZOXFEOF
log_success "Fish zoxide configured"

# Zsh: source zoxide init from .zshrc (if zsh is installed)
if [[ -f "$HOME/.zshrc" ]]; then
    sed -i "/${ZOXIDE_MARKER}/,/# end iNiR zoxide/d" "$HOME/.zshrc"
    cat >> "$HOME/.zshrc" << ZOXZEOF

${ZOXIDE_MARKER}
if command -v zoxide &>/dev/null; then
  eval "$(zoxide init zsh${ZOXIDE_CMD_FLAG:+ }${ZOXIDE_CMD_FLAG})"
fi
# end iNiR zoxide
ZOXZEOF
    log_success "Zsh zoxide configured"
fi
