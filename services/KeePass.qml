...
Singleton {
    ...
    // Persisted in config so the "stay unlocked" duration survives restarts.
    readonly property int cacheTtl: Config.options?.keepass?.cacheTtl ?? 300
    property int remainingTime: 0

    // Whether a usable Secret Service is reachable. When false, the master
    // password is kept in memory for the session so copy/reveal still work
    // (probed once at startup; assume available until proven otherwise).
    ...
    Component.onCompleted: {
        availabilityProc.exec(["sh", "-c", "command -v keepassxc-cli >/dev/null 2>&1"])
        keyringProbeProc.exec({ command: [scriptPath, "check-keyring"] })
        // Drop any master password left in the keyring by a previous session or
        // a crash: the shell always starts locked, so a cached secret would be
        // dead weight (there is no auto-unlock from the keyring).
        staleClearProc.exec({ command: [scriptPath, "clear-all"] })
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            id: keyringProbeCollector
            onStreamFinished: root.keyringAvailable = (keyringProbeCollector.text.trim() === "yes")
        }
    }

    Process { id: staleClearProc }
    ...
    Process {
        ...
        onExited: (exitCode) => {
            ...
            if (exitCode === 0) {
                ...
                root.unlocked = true
                root.lastError = ""
                // Keep the password in memory only when the keyring can't cache
                // it, so subsequent copy/reveal still have a source to feed.
                if (root.keyringAvailable) root.password = ""
            } else {
                root.entries = []
                root.unlocked = false
    ...
    Process {
        ...
        onExited: (exitCode) => {
            ...
            root.open = true
            root.addMode = true
            // Keep the unlocked state when the DB is already open — the keyring
            // still holds the password, so dropping it here only flashed the
            // unlock tab even though we could save right away.
            if (!root.unlocked) root.resetSensitive()
            root.refreshEntries()
        }
    }
